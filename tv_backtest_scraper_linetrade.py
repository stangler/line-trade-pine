# tv_backtest_scraper_linetrade.py — ライントレード戦略用
import asyncio
import csv
import re
...
USER_DATA   = r"C:\Temp\tv-profile-pw"
WAIT_RECALC = 6
WAIT_SEARCH = 1.5
EXCHANGE    = "TSE"
# ==========================
...
def parse_dw(dw_data: list) -> dict:
    """Data Window項目をdict化"""
    return {row[0]: row[1] for row in dw_data if len(row) == 2}
...
def save_csv(strategy_data: list, dw_data: list, symbol: str):
    ...
    # 集計行を生成
    def row_stats(prefix_trades, prefix_wins):
        t = safe_int(dw.get(prefix_trades, 0))
        w = safe_int(dw.get(prefix_wins, 0))
        return t, w, win_rate(w, t)
    ...
    with open(fn, "w", newline="", encoding="utf-8-sig") as f:
        ...
        w.writerow([])
        w.writerow(["=== 方向別サマリー ===", "", ""])
        w.writerow(["方向", "トレード数", "勝数", "勝率"])
        lt, lw, lwr = row_stats("[Long]Trades",  "[Long]Wins")
        st, sw, swr = row_stats("[Short]Trades", "[Short]Wins")
        w.writerow(["Long",  lt, lw, lwr])
        w.writerow(["Short", st, sw, swr])

        w.writerow([])
        w.writerow(["=== モード×方向サマリー ===", "", ""])
        w.writerow(["モード", "方向", "トレード数", "勝数", "勝率"])
        for mode, prefix in [("ブレイクアウト", "BO"), ("リバウンド", "RB"), ("トレンドフォロー", "TF")]:
            for direction in ["Long", "Short"]:
                t, wn, wr = row_stats(f"[{prefix}]{direction}Trades", f"[{prefix}]{direction}Wins")
                w.writerow([mode, direction, t, wn, wr])

        w.writerow([])
        w.writerow(["=== Data Window (raw) ===", ""])
        w.writerow(["指標", "値"])
        w.writerows(dw_data)
...
async def scrape_strategy(page) -> list:
    # ライントレード戦略のStrategy Testerキーワード
    keywords = [
        "総損益", "最大ドローダウン", "プロフィットファクター",
        "純利益", "総取引数", "勝率",
    ]
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print("  2. ライントレード戦略適用済みチャートを開く（5分足）")
